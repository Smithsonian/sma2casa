...
def read(dataDir):
    ...
    for rec in range(nBlRecords):
        if ((rec % 10000) == 0) and verbose:
            print '\t processing record %d of %d (%2.0f%% done)' % (
            rec, nBlRecords, 100.0 * float(rec) / float(nBlRecords))
            sys.stdout.flush()
        ...
        if len(data) == blRecLen:
            ...
            if isb == 0:
                blDictL[blhid] = (
                inhid, ipol, ant1rx, ant2rx, pointing, irec, u, v, w, prbl, coh, avedhrs, ampave, phaave,
    ...
    if gotTsysFile:
        ...
        if newFormat:
            ...
            for bl in blDictL:
                ...
                ant1 = blDictL[bl][15]
                ant2 = blDictL[bl][16]
                blTsysDictL[bl] = (antLowDict[(inhid, ant1)], antHighDict[(inhid, ant1)], antLowDict[(inhid, ant2)],
                                   antHighDict[(inhid, ant2)])
            for bl in blDictU:
                inhid = blDictU[bl][0]
                ant1 = blDictU[bl][15]
                ant2 = blDictU[bl][16]
                blTsysDictU[bl] = (antLowDict[(inhid, ant1)], antHighDict[(inhid, ant1)], antLowDict[(inhid, ant2)],
                                   antHighDict[(inhid, ant2)])
    ...
    if swappingTsys:
        ...
        for bl in blDictL:
            blTsysDictL[bl] = (mapping[(blDictL[bl][0], tsysMapping[blDictL[bl][15]])][0],
                               mapping[(blDictL[bl][0], tsysMapping[blDictL[bl][15]])][1],
                               mapping[(blDictL[bl][0], tsysMapping[blDictL[bl][16]])][0],
                               mapping[(blDictL[bl][0], tsysMapping[blDictL[bl][16]])][1])
        mapping = {}
        for bl in blDictU:
        ...
        # Now use that mapping table to perform the Tsys substitution
        for bl in blDictU:
            blTsysDictU[bl] = (mapping[(blDictU[bl][0], tsysMapping[blDictU[bl][15]])][0],
                               mapping[(blDictU[bl][0], tsysMapping[blDictU[bl][15]])][1],
                               mapping[(blDictU[bl][0], tsysMapping[blDictU[bl][16]])][0],
                               mapping[(blDictU[bl][0], tsysMapping[blDictU[bl][16]])][1])
    ...
    for rec in range(nSpRecords):
        if ((rec % 100000) == 0) and verbose:
            print '\t processing record %d of %d (%2.0f%% done)' % (
            rec, nSpRecords, 100.0 * float(rec) / float(nSpRecords))
            sys.stdout.flush()
        data = f.read(spRecLen)
            ...
            if rightRx:
                ...
                try:
                    ...
                    if blSidebandDict[blhid] == 0:
                        wt = wt / (max(blTsysDictL[blhid][0], blTsysDictL[blhid][1]) * max(blTsysDictL[blhid][2],
                                                                                           blTsysDictL[blhid][3]))
                        if iband not in spSmallDictL:
                            spSmallDictL[iband] = (nch, fres * 1.0e6, fsky * 1.0e9, rfreq * 1.0e9)
                    else:
                        wt = wt / (max(blTsysDictU[blhid][0], blTsysDictU[blhid][1]) * max(blTsysDictU[blhid][2],
                                                                                           blTsysDictU[blhid][3]))
...
parser = argparse.ArgumentParser(
    description='Convert an SMA MIR format dataset into a set of FITS-IDI files which may be imported into CASA.')
...
parser.add_argument('-p', '--percent',
                    help='Percentage to trim on band edge (default = %0.0f) - implies -t' % (edgeTrimFraction * 100.0),
                    type=float)
group1.add_argument('-P', '--PythonOnly', help='Do not use the C module "makevis" - use pure Python (slow)',
                    action='store_true')
parser.add_argument('-r', '--receiver', help='Specify the receiver (required for multi-receiver tracks)', type=int,
                    choices=[230, 240, 345, 400])
parser.add_argument('-R', '--RxFix', help='Force the script to treat the track as a single receiver track',
                    action='store_true')
        ...
        if (band in chunkList) and (sb in sidebandList):
            ...
            if fakeChunk:
                if sb == 0:
                    ...
                    centerFSky = spSmallDict[newChunks[band][0]][2] + (midCh - nCh * 0.5) * \
                                 spSmallDict[newChunks[band][0]][1]
                    if (newChunks[band][0]) < 49:
                        lowestFSky = spSmallDict[newChunks[band][0]][2] - spSmallDict[newChunks[band][0]][1] * 0.5 - \
                                     spSmallDict[newChunks[band][0]][1] * spSmallDict[newChunks[band][0]][0] * 0.5 + (
                                                 startCh + freqInc) * abs(spSmallDict[newChunks[band][0]][1])
                ...
                spSmallDict[band] = (
                int(nChNew / newChunks[band][3]), spSmallDict[newChunks[band][0]][1] * newChunks[band][3],
                ...
                if band <= 5:
                    if sb == 0:
                        lowestFSky = spSmallDict[band][2] - spSmallDict[band][1] * 0.5 - abs(spSmallDict[band][1]) * \
                                     spSmallDict[band][0] * 0.5
                        print 'Lower', band, lowestFSky, spSmallDict[band][0], spSmallDict[band][1], spSmallDict[band][
                            2]
                    else:
                        lowestFSky = spSmallDict[band][2] + spSmallDict[band][1] * 0.5 - abs(spSmallDict[band][1]) * \
                                     spSmallDict[band][0] * 0.5
                        print 'Upper', band, lowestFSky, spSmallDict[band][0], spSmallDict[band][1], spSmallDict[band][
                            2]
                elif band < 49:
                    if sb == 0:
                    ...
                    if sb == 0:
                        lowestFSky = spSmallDict[band][2] - spSmallDict[band][1] * 0.5 + spSmallDict[band][1] * \
                                     spSmallDict[band][0] * 0.5
                    else:
                        lowestFSky = spSmallDict[band][2] + spSmallDict[band][1] * 0.5 - spSmallDict[band][1] * \
                                     spSmallDict[band][0] * 0.5
            ...
            antNames = np.array(
                ['SMA1    ', 'SMA2    ', 'SMA3    ', 'SMA4    ', 'SMA5    ', 'SMA6    ', 'SMA7    ', 'SMA8    '])
            antList = []
            velList = []
            antNumList = []
            antStyleList = []
            ...
            c4 = fits.Column(name='TOTAL_BANDWIDTH', format='1E',
                             array=[abs(spSmallDict[band][1]) * float(spSmallDict[band][0])])
            if sideBand == 'Upper':
            ...
            if verbose:
                if fakeChunk:
                    print 'Creating UV_DATA table for synthetic band ', band, sideBand, ' with ', (1 + newChunks[band][
                        2] - newChunks[band][1]) / newChunks[band][3], ' channels'
                else:
                    print 'Creating UV_DATA table for band ', band, sideBand, ' with ', spSmallDict[band][
                        0], ' channels'
            uList = []
            vList = []
            wList = []
            ...
            while (scanOffset < visFileLen) and (scanNo < maxScan):
                ...
                if scanNo > 0:
                    for bl in blKeysSorted[blPos:]:
                        ...
                        if (blDict[bl][0] == scanNo) and rightRx:
                            ...
                            if (band, bl) in spBigDict:
                                ...
                                if useMakevis:
                                    if fakeChunk:
                                        matrixEntry = makevis.convert(nChannels, spSmallDict[band][4], scale, dataoff,
                                                                      newFormat, weight, False, firstGoodChannel,
                                                                      lastGoodChannel, reverseChannelOrder,
                                                                      newChunks[band][3],
                                                                      newChunks[band][1] / newChunks[band][3],
                                                                      newChunks[band][2] / newChunks[band][3])
                                    else:
                                        matrixEntry = makevis.convert(nChannels, nChannels, scale, dataoff, newFormat,
                                                                      weight, trimEdges, firstGoodChannel,
                                                                      lastGoodChannel, reverseChannelOrder, 1, 0,
                                                                      nChannels - 1)
                                    if rightRx:
                                    ...
                                    for i in range(0, nChannels):
                                        ...
                                        if (weight <= 0.0) or (
                                                trimEdges and (not (firstGoodChannel <= i <= lastGoodChannel))):
                    ...
                    if (not foundBlEntry) or (not foundSpEntry):
                        print 'Something not found scanNo = %d, band = %d, foundBl = %d, foundSp = %d' % (
                        scanNo, band, foundBlEntry, foundSpEntry)
