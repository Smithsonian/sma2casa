...
for sideband in sidebandsToProcess:
    ...
    for i in chunksToProcess:
        ...
        for ii in range(len(cards)):
            try:
                if (' was on pad ' in cards[ii]) and not ('Not used' in cards[ii]) and not (
                        'CSO' in cards[ii]) and not ('JCMT' in cards[ii]):
                    tok = cards[ii].split()
                    ant = int(tok[1])
                    pad = tok[5] + ' ' + tok[6]
                    stations[ant - 1] = pad
    ...
    if len(sourceList) > 0:
        for concatvis in vis:
            ...
            tb.create(concatvis + '/SOURCE', tabledesc=sourceTableDesc, dminfo=sourceDMInfo,
                      nrow=len(sourceList) * len(bandList))
            iArray = []
